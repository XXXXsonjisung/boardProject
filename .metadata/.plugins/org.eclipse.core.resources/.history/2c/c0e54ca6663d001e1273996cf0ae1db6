...
public class EmployeeView {
	...
	do {
		try {
			...
			// selectJobAvgSalary()
			// DB 조회 결과를 HashMap<String, Double>에 옮겨 담아서 변환
			...
			case 6: scdEmployee(null); break;
			case 7: scsupEmployee(); break;
			case 8: dsallEmployee(); break;
			case 9: emnoEmployee(); break;
			case 10: jobsaEmployee(); break;
	 ...
	 * 6번
	 */
	private void scdEmployee(List<Employee> empList) {
		
		

		System.out.println("조회하실 부서명을 적어주세요 : ");
	...
	private void updateEmployee() throws Exception{
		...
		if(result > 0 ) {
			System.out.println("사원정보 수정되었습니다.");
		} else {
			System.out.println("사번이 일치하는 직원이 없음");
		}
	}
...
//		return empId;
	...
	private void PrintOne(Employee emp) {
		// TODO Auto-generated method stub
	...
	System.out.println("직급 코드(J1~J7)");
	String jobCode = sc.next();
	
	System.out.print("급여등급(S1~S6) : ");
	String salevel = sc.next();
	...
	Employee emp = new Employee(empId,empName,empNo,email,phone,deptCode,
			jobCode,salevel,salary,Bonus,managerId);
	
	int result = service.insertEmployee(emp);
	
	if(result > 0) {
		System.out.println("사원 정보 추가성공");
	} else {
